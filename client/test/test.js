...
contract('DVideo', ([deployer, owner]) => {
  let dvideo
  ...
  describe('videos', async () => {
    ...
    before(async () => {
      result = await dvideo.uploadVideo(hash, 'Video title', { from: owner })
    ...
    it('creates videos', async () => {
      ...
      await dvideo.uploadVideo('', 'Video title', { from: owner }).should.be.rejected;

      // FAILURE: Video must have title
      await dvideo.uploadVideo('Video hash', '', { from: owner }).should.be.rejected;
    })
    ...
    it('lists videos', async () => {
      ...
      assert.equal(video.owner, owner, 'owner is correct')
    })
  })
})